...
void MyMain(string argument, double elapsedNow) {
...
public bool logic(ref Vector3D grav, ref Vector3D speedLocal, out Nullable<Vector3D> heading, out Vector3D targetSpeedLocal) {
  ...
  bool dockPart1 = docking && dockingApproach;
  bool dockPart2 = docking && !dockingApproach;
  ...
  Print("TGT: ("+targetName+") " + new MyVectD(targetLoc), true );
  ...
  if(dockPart1) {
    ...
    diff = targetLoc - (myConnector.GetPosition() + dockOffset);
    margin = 1.0;
  } else if(dockPart2) {
...
public void parseTarget() {
    String[] parts = targets[tgtIndex].Split(':');
    targetName = parts[1];
    targetLoc = new Vector3D(float.Parse(parts[2]), float.Parse(parts[3]), float.Parse(parts[4]));
    docking = targetName.StartsWith("CONN");
...
public T FindFirstWithPrefixOrAny<T>() {
